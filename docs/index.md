...
**Present**: the upstream Odoo codebase at branch `19.0`, a released stable series — `odoo/release.py` declares `version_info = (19, 0, 0, FINAL, 0, '')` — carrying 605 addons, including the base `account` module inherited from upstream. No source file in that tree was modified by the change set this documentation belongs to.
