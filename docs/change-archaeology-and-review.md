...
| 6 | `catalog-info.yaml` | UPDATE | In place at the repository root | **Landed** — the documentation link, lifecycle, type, tag and label rework, the first link's icon and the authoritative description with the commit that publishes this revision. See subsection 6.3 | S1-01 … S1-04, S1-06 and the descriptor legs of S5-01 … S5-03 |
...
**Findings that remain open by design, and where they will close.** The descriptor and the ignore file were both deferred to the configuration checkpoint that owns them. The descriptor half has since landed, closing **S1-01 … S1-04**, **S1-06** and the descriptor leg of **S5-01 … S5-03**; the ignore half has not, so **S2-06** alone is still open at the time of writing. Subsection 6.1 pins the status of every transformation and subsection 6.3 measures each one. Nothing in this record should be read as claiming S2-06 closed.
