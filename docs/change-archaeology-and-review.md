...
| `commits` | **49** | The guide's *Total Commits* figure of 47 is wrong by two; the authoritative figure is 49 |
...
The table below carries **one instance of the invariant per commit in the remediation series**, and the series is complete through `bc1e4f9f5c1b`. Row numbers record the order in which rows were *added to this table*, not the order in which the commits were authored; the *Check* column therefore names each commit explicitly. In commit order — `git log --reverse --format=%h c789a23602c2..bc1e4f9f5c1b` — the series runs `7a543d4b3eea` (NR-14), `9e7ea620a69e` (NR-15), `f9bee9fb4234` (NR-10 … NR-12), `39efbcd2b81b` (NR-13), `712739da9e8a` (NR-16), `08a62fff8802` (NR-17), `f6381d44099c` (NR-18) and `bc1e4f9f5c1b` (NR-19). No earlier row was deleted or re-pointed to make room for the six that were added later, which is the obligation the closing paragraphs of this subsection impose.
...
| NR-19 | The same three assertions at the commit that closed the ignore leg, the last commit of the series | `git diff --name-only 7bd7718bcd4c..bc1e4f9f5c1b` with the two pathspecs of NR-10 and NR-11, and `git diff --name-only c789a23602c2..bc1e4f9f5c1b` | **0 paths**, **0 paths**, and **eight** in-scope paths — `.gitignore` joins the set. Under `--no-renames` the figure is **10**, which is exactly the ten transformations of subsection 6.1, neither more nor fewer |

**The invariant, and the obligation it places on whoever extends this record.** The property being asserted is per-commit and does not depend on which commit is currently the tip: *for every commit `C` in the remediation series, `git diff --name-only 7bd7718bcd4c..C` contains no path matching a source extension, no path under an upstream prefix, and no path outside the ten in-scope paths of subsection 6.1.* The measurements above are its instances at all eight commits of the series to date — `7a543d4b3eea`, `9e7ea620a69e`, `f9bee9fb4234`, `39efbcd2b81b`, `712739da9e8a`, `08a62fff8802`, `f6381d44099c` and `bc1e4f9f5c1b`. **Any later remediation commit must add its own pinned row rather than re-point these to a moving reference** — the failure mode this subsection exists to prevent. Rows NR-14 … NR-19 are that obligation being discharged: at the revision NR-13 was written the table pinned only two of the then-four commits, and the six rows added since close the gap additively, without deleting, renumbering or re-pointing NR-10 … NR-13.
...
**Where the lag stands at this revision.** The pinned rows reach `bc1e4f9f5c1b`, the tip at which this revision was authored, so the series is complete except for the single commit that publishes this revision itself — the one row that cannot exist yet, for the reason just given. That residue is exactly one commit and no more, and the invariant stated above already quantifies over it. Whoever writes the next revision inherits precisely one obligation from this subsection: add a row pinning the commit that published this one.
...
| ID | Severity · class | Finding | Remediation |
|----|------------------|---------|-------------|
| S4-01 | **Blocker** · D3 | The delivery statistics — 47 commits, 81 files created, 27,707 lines added, 43 documentation files, 36 module source files — describe a different branch entirely. This branch carries 13 commits and 6 files. **Two of the five figures are additionally wrong about the branch they do describe**: the commit count is 49, not 47, and the addition count is 27,905, not 27,707 | Re-attribute the table to the pull request and branch it describes; correct **both** wrong figures against the authoritative record — **49** commits and **27,905** additions; add a row stating what this branch actually contains. The module accounting is likewise re-derived from the authoritative 81-entry file list rather than restated: **36** module files totalling **6,667** additions, allocated as 21 Python files / 3,763, 12 XML / 2,292, 2 SCSS / 596 and 1 CSV / 16 |
...
- **S4-01** — *Evidence:* [doc/project-guide.md:L378-L388]; PR record `changed_files` 81, `commits` 49 and `additions` 27,905 — *Acceptance check:* Every statistic names the branch it applies to, and every figure matches the authoritative record. The per-language subtotals are the check on the module accounting: summing the file list's own `additions` by extension must reproduce the table's Python 3,763, XML 2,292 and SCSS 596 exactly, and the four subtotals must sum to 6,667 across 36 files. The residual 21,238 additions belonging to the 45 non-module entries are obtained **by subtraction** from the authoritative 27,905, because the file-list endpoint reports no additions for 19 of those 45 entries while reporting them for all 36 module entries
...
- **Diagram text alternatives inside the two relocated documents.** Seven prose alternatives — two in the project guide and five in the specification — were added to those documents at `08a62fff8802` as an accessibility measure, and they have been **withdrawn** by the restoration described in subsection 6.8. The reason is precedence, not a judgement about their merit: the plan enumerates the corrections authorised for each of those two documents, that enumeration does not include them, and under **ST-9** an accessibility heuristic does not override the plan — the same rule finding D31 invokes for the heading permalinks. They are recorded here as **requiring separate authorisation** rather than silently dropped, and re-adding them is a one-commit change once that authorisation exists. The single alternative in subsection 3.1 of *this* document is unaffected and remains in place: this file is new authorship whose content the plan does not fix, so no protected enumeration governs it.
- **Escaped pipe characters inside code spans in three specification tables.** Three cells in the specification's validation tables carry a shell pipeline written as `` `… \| wc -l` ``. Python-Markdown does not unescape `\|` inside a code span in a table cell, so each renders with a literal backslash and is not copy-pasteable — the same rendering defect that caused the two documentation-build commands repaired in subsection 6.8 to be moved out of a table and into a fenced block rather than repaired in place. These three cells are **protected content that no finding names**, so they are left exactly as authored: repairing them would be precisely the unenumerated edit to protected content that subsection 6.8 exists to record as a regression. They are noted so that a future authorised pass can move them into fenced blocks in one edit.
...
**Consequence carried forward, and now discharged.** The documentation root and the edit reference landed first, leaving the lockstep unit two-thirds closed while [catalog-info.yaml:L25] still pointed at the 404 path. That split has since been closed by the configuration checkpoint that owns the descriptor: the outbound documentation link resolves to the same `docs/` root the edit reference already names, so the three cannot disagree, and the portal never observes an intermediate state in which the root has moved but the link has not. Each of the **eight** keys this configuration carries — `docs_dir`, `site_name`, `site_description`, `repo_url`, `edit_uri`, `nav`, `plugins`, `markdown_extensions` — was confirmed to sit inside the publisher's allowlist of supported keys, so none is stripped when the portal sanitises the file before building.
...
| 2 | Catalog documentation link | the `blitzy/documentation` tree path — HTTP **404** | the relocated `docs` directory on the published branch — HTTP **200** | [catalog-info.yaml:L25] | Landed with row 6 of subsection 6.1 |
...
### 6.8 The one regression in the series, and its restoration

Every other subsection of this section records a repair. This one records a **regression introduced by the remediation itself**, and the restoration that undid it. It is here rather than omitted because a record that documented only its own successes would be worth less than no record at all — and because the regression is instructive: it is the exact failure mode the frozen-plan rule exists to prevent, committed by an agent acting in good faith on a genuine accessibility signal.
...
| Specification | 18 intra-word emphasis markers were replaced with whole-word emphasis across the two INVEST tables | Protected table content; the `**S**mall` form is the acronym device the tables are built on |
...
| Project guide | Five Development Guide shell fences were rewritten with line continuations | Protected content — and one of those five fences was *also* factually broken, which is why the restoration is not the whole story |
...
**The restoration, and why the restore point is `9e7ea620a69e`.** Both documents were restored **byte-exactly** to their state at `9e7ea620a69e`, verified by `git diff --quiet 9e7ea620a69e -- <path>` returning clean for each. That commit is the correct restore point for two independent reasons. It already carries every correction the plan authorises for these two documents — the single top-level heading with its exact required text, the status banners, the qualified version constraint, the corrected source-line citation, the branch-scoped statistics and the dependency-ordered phases — so restoring to it forfeits none of the authorised work. And `git diff --stat 9e7ea620a69e..bc1e4f9f5c1b` for these two paths is **byte-identical** to `git diff --stat 08a62fff8802~1..08a62fff8802` for them, which proves no commit after `08a62fff8802` touched either file and therefore that the restoration reverts exactly the unauthorised edits and nothing else.

**What was deliberately *not* restored.** Two of the four artifacts that commit touched were left as it left them, for stated reasons. `mkdocs.yml` changed only inside `plugins`, one of the eight keys the remediation is authorised to set, so that edit was within scope. And *this* document is new authorship created by the remediation rather than plan-fixed content, so no protected enumeration governs it; its own accessibility additions stand, which is why the surviving text-alternative population is 1 rather than 0.

**Authorised corrections layered on top of the restored baseline.** Restoring alone would have reinstated real defects, because some of the content the unauthorised edits touched was independently wrong — most sharply, the Development Guide was both protected *and* not runnable. The corrections below were therefore applied to the restored files. Each closes a specific reported defect, each is minimal, and none touches a protected region that no finding names.

| Document | Correction | Evidence it rests on |
|----------|-----------|----------------------|
| Project guide | *Total Commits* 47 → **49** | The PR record's own `commits` field, tabulated in subsection 3.6 |
| Project guide | The module accounting regenerated to **36** files and **6,667** additions with per-directory rows and subtotals, replacing a table whose rows summed to 32 and a paragraph claiming a 3,997-against-3,763 residual that could not be re-measured | Summing the authoritative 81-entry file list's own `additions` by extension reproduces the delivery table's Python 3,763, XML 2,292 and SCSS 596 exactly, so the residual was always measurable and the earlier claim was false |
| Project guide | PostgreSQL prerequisite `12+` → **`13+`**, with a new column recording how each version bound is actually enforced | `MIN_PG_VERSION = 13` at [odoo/release.py:L39-L41]; the per-connection warning at [odoo/sql_db.py:L693-L694]; the hard minimum-Python assertion at [odoo/init.py:L9] against the *warning-only* maximum-Python path at [odoo/cli/server.py:L69-L73] — a distinction the correction states rather than blurs |
| Project guide | The clone target repointed from upstream to this fork | The branch the guide then checks out returns HTTP 200 in the fork and HTTP **404** upstream, so the sequence as written could not complete |
| Project guide | The database role the guide creates is now wired into the documented flow through `PGUSER` and `--db_user` | [odoo/tools/config.py:L371-L374] declares those environment names; [odoo/cli/server.py:L63] shows the resolution order. Without this the guide created a role nothing used |
| Specification | Four opaque `<cite index="…">` elements replaced with Markdown footnotes | Two of the four quotations were traced to identifiable public sources and are retained **as quotations** with resolvable references; the other two could not be traced to any publication and are therefore **paraphrased in the document's own voice**, with the footnote declaring the paraphrase rather than implying a source that was never found |
| Specification | The npm coordinate corrected to the scoped **`@mermaid-js/mermaid-cli`** | The registry confirms the scoped package publishes the cited version; the unscoped name's latest release is 0.2.4 from 2014 and is deprecated, so the original coordinate named a package that has no such version |
| Specification | The documentation-build commands made tickets-rooted and moved out of a table into a fenced block; the preview command relabelled long-running and blocking | The command as written built the current documentation root rather than the tree the surrounding section describes. The blocking behaviour was **measured**, not assumed: run under a six-second timeout the preview command exits 124 while logging that it is watching and serving. The move out of the table is itself required, because Python-Markdown does not unescape an escaped pipe inside a code span in a table cell |
| Specification | Three alternation-based content checks replaced with a per-file all-token check carrying an exact expected count | Each original check passed a file that contained only one of its required clauses, and none asserted a total. The replacement was executed against three fixtures — conforming, one token deleted, one file missing — and reported pass, a named per-file failure, and a count mismatch respectively |
| Specification | Time-boxed sizing language replaced with bounded-effort wording at two locations, preserving the intra-word emphasis device | Calendar and iteration scheduling in platform-authored planning content is a contract breach under ST-7, which is also why S4-05 exists |
| Specification | "Odoo 19.0 (Master)" → **"Odoo 19.0 stable"**, and 607 → **605** addon directories | `version_info = (19, 0, 0, FINAL, 0, '')` at [odoo/release.py:L15], with `FINAL` the last of the release levels declared at [odoo/release.py:L3], so the series is released and not a trunk. The addon count agrees across three pinned measurements — `git ls-tree -d --name-only bc1e4f9f5c1b addons/`, the same query at the baseline `7bd7718bcd4c`, and a working-tree directory count — all **605** |

**The lesson, stated rather than implied.** Every unauthorised edit in the table above would pass a reasonable reviewer's taste test. Reordering pie slices, spacing lists, wrapping long commands and adding prose alternatives are all improvements in isolation. That is precisely why ST-9 is not a formality: the boundary between *authorised correction* and *unauthorised improvement* cannot be drawn by judging each edit on its merits, only by consulting the enumeration. The regression is recorded here, with its restoration and its evidence, so that the next agent to feel the same pull has this subsection to read first.

**One honest limit on this subsection.** Like every pinned reference in this record, it cannot cite the commit that publishes it — subsection 4.2 explains why, and its NR-19 row pins the last commit this revision *can* name. The restoration and the corrections above are therefore verifiable against `9e7ea620a69e` and `08a62fff8802`, both of which are cited, and the row pinning their publication is the single obligation this revision leaves to the next.
...
**The two findings that were once deferred are now both closed.** The descriptor and the ignore file were each deferred, in turn, to the configuration checkpoint that owns them. The descriptor half landed at `f6381d44099c`, closing **S1-01 … S1-04**, **S1-06** and the descriptor leg of **S5-01 … S5-03**; the ignore half landed at `bc1e4f9f5c1b`, closing **S2-06**. **No review finding of the twenty-nine remains open on repository grounds.** Subsection 6.1 pins the status of every transformation to the commit that carried it and subsection 6.3 measures each one, so nothing in this record should now be read as claiming S2-06 — or any other finding — still deferred. What remains open is of two kinds only, and neither is a repository defect: the two portal-side prerequisites in the table above, which no file edit here can discharge, and the theme-level and plugin-level runtime defects in subsection 7.1, each labelled either genuinely out of reach or reachable-but-declined with its lever named.
